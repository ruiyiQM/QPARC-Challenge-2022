...
ps.multiplicity = 3  # required (from ver. 0.18.1)
...
input.ansatz.randomness_variance = 0.25
...
input.cost_function.s2_number_weight= 100.
input.cost_function.sz_number_weight= 100.
input.cost_function.particle_number_weight= 100.
# exact simulator (state vector)
input.quantum_device.type = "EXACT_SIMULATOR"

job=client.submit(input)
save_job(job, filename="cas22.json")
